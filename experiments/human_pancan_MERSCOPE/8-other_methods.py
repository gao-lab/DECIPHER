...
# %% [markdown]
# # Run other spatial methods in downsampled human pan-cancer spatial atlas
#
...
# %%
tmp = sc.read_h5ad('./results/adata_analysis.h5ad')
adata_sub.obs['region'] = tmp.obs['region'][subsample_idx]
del tmp

# %%
os.makedirs('./results/other_methods_compare', exist_ok=True)
...
# %%
edge_index = build_graph(adata_sub.obsm['spatial'], k=20, mode='knn')
torch.save(edge_index, './results/other_methods_compare/edge_index_k20.pt')
...
sc.pl.umap(adata_sub, color=['cell_type', 'region', 'Tissue_0'], ncols=1)
...
# %%
# Spatial graph 
STAGATE_pyG.Cal_Spatial_Net(adata_sub, k_cutoff=20, model='KNN')
...
# %%
adata_sub.obsm['X_stagate'] = adata_sub.obsm['STAGATE']
